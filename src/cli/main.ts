...
import type { Status } from "../models/task.js";
...
switch (command) {
    case "create": {
        ...
        if (!title) { console.log("Titre manquant."); help(); process.exit(1); }
    ...
    case "list": {
        const status = args[0] as Status | undefined;
    ...
    case "delete": {
        ...
        if (!id) { console.log("ID manquant."); help(); process.exit(1); }
    ...
    case "update": {
        ...
        const patch: any = {};
        if (titleIdx !== -1 && args[titleIdx + 1]) patch.title = args[titleIdx + 1];
        if (descIdx  !== -1 && args[descIdx + 1])  patch.description = args[descIdx + 1];
        if (statIdx  !== -1 && args[statIdx + 1])  patch.status = args[statIdx + 1];
    ...
    case "mark": {
        ...
        if (!id || !status) { console.log("Usage: mark <id> <todo|in-progress|done>"); process.exit(1); }
